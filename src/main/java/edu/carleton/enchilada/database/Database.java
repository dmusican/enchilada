...
public abstract class Database {
    ...
    protected void loadConfiguration(String dbname) {
        ...
        try {
            ...
            while (scan.hasNext()) {
                ...
                if (tag.equalsIgnoreCase("db_url:")) {
                    url = val;
                } else if (tag.equalsIgnoreCase(dbname + "_db_port:")) {
                    port = val;
                }
            }
            scan.close();
    ...
    public abstract int insertParticle(
            String dense, java.util.Collection<ATOFMSPeak> sparse,
...
//	}
    ...
    public String getVersion() {
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Can't understand what state the DB is in. (has version field but no value)");
        }
        return version;
    }
    ...
    public HashMap<Integer, ArrayList<Integer>> getSubCollectionsHierarchy(Collection collection) {
        ...
        try {
            ...
            ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                     "SQL Exception grabbing subchildren in GetImmediateSubCollections.");
    ...
    public String getCollectionName(int collectionID) {
        ...
        try {
            ...
            ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                     "Error retrieving the collection name for collectionID " + collectionID);
     ...
     */
    public int createEmptyCollection(
            String datatype,
            int parent,
            String name,
            String comment,
     ...
     */
    public int[] createEmptyCollectionAndDataset(
            String datatype, int parent,
    ...
    public int copyCollection(Collection collection, Collection toCollection) {
        ...
        try {
            ...
            ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                     "SQL Exception copying collection " + collection.getName());
            throw new ExceptionAdapter(e);
        }
    }
    ...
    public boolean moveCollection(
        ...
        try {
            ...
            ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                     "SQL Exception moving the collection " + collection.getName());
    ...
    public boolean addAtom(int atomID, int parentID) {
        ...
        try {
            ...
            ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                     "SQL Exception adding atom " + atomID + "to AtomMembership.");
    ...
    public boolean orphanAndAdopt(Collection collection) {
        try {
            ...
            if (parentID == -1)
                ...
                ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                         "Cannot perform the Orphan And Adopt operation on root level collections.");
    ...
    public void atomBatchInit() {
        try {
            ...
            ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                     "SQL Exception occurred initializing AtomBatch functionality.");
            e.printStackTrace();
        }
    }
     ...
     */
    public Vector<Vector<Object>> updateParticleTable(
            Collection collection, Vector<Vector<Object>> particleInfo, int lowIndex, int highIndex) {
    ...
    public String getAtomDatatype(int atomID) {
        ...
        try {
            ...
            ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                     "SQL Exception getting the datatype for atom " + atomID);
    ...
    public java.util.Date exportToMSAnalyzeDatabase(
        ...
        try (
            ...
            dataSetsTable.addRow(newName, startTime, endTime, hitParticles, 0, 0, 0, 0, comment);

            // Delete all rows from Particles where newName is present
        ...
        // Ensures that temp.ParticlesToExport gets dropped, without locking errors
        closeConnection();
        openConnection();
        try (
                Statement stmt = con.createStatement();
    ...
    private class AtomInfoOnlyCursor
        ...
        public void reset() {
            try {
                partInfRS.close();
                partInfRS = stmt.executeQuery("SELECT " + getDynamicTableName(DynamicTable.AtomInfoDense,
                                                                              collection.getDatatype()) + ".AtomID, OrigFilename, ScatDelay," +
                                                      " LaserPower, [Time], Size FROM " + getDynamicTableName(
                        DynamicTable.AtomInfoDense, collection.getDatatype()) + ", InternalAtomOrder WHERE" +
                                                      " InternalAtomOrder.CollectionID = " + collection.getCollectionID() +
                                                      " AND " + getDynamicTableName(DynamicTable.AtomInfoDense,
                                                                                    collection.getDatatype()) + ".AtomID = InternalAtomOrder.AtomID");
            } catch (SQLException e) {
    ...
    private class SQLCursor extends AtomInfoOnlyCursor {
        ...
        public SQLCursor(Collection col, String where, Database db) {
            ...
            try {
                ...
                ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                         "SQL Exception retrieving data through a SQL cursor.");
                e.printStackTrace();
            }
        }
    ...
    private class PeakCursor extends AtomInfoOnlyCursor {
        ...
        public PeakCursor(Collection col) {
            ...
            try {
                ...
                ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                         "SQL Exception retrieving data through a Peak cursor.");
                e.printStackTrace();
            }
        }
    ...
    private class RandomizedCursor extends BinnedCursor {
        ...
        public RandomizedCursor(Collection col) {
            ...
            try {
                ...
                ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                         "SQL Exception retrieving data through a Randomized cursor.");
     ...
     */
    public CollectionCursor getSQLAtomIDCursor(
            Collection collection,
            String where) {
     ...
     * get method for SQLCursor.
     */
    public CollectionCursor getSQLCursor(
            Collection collection,
            String where) {
        ...
        try {
            ...
            ResultSet rs
                    = stmt.executeQuery("SET NOCOUNT ON " +
                                                "INSERT ValueMaps (Name) Values('" + removeReservedCharacters(
                    name) + "') " +
                                                "SELECT @@identity " +
                                                "SET NOCOUNT OFF");
            rs.next();
    ...
    public Vector<int[]> getValueMapRanges() {
        ...
        try {
            ...
            while (rs.next())
                valueMapRanges.add(
                        new int[]{rs.getInt("ValueMapID"), rs.getInt("Value"), rs.getInt("Low"), rs.getInt("High")});
            rs.close();
            stmt.close();
        } catch (SQLException e) {
        ...
        try {
            ...
            if (c.getCollectionID() == basis.getCollectionID()) {
                ...
                stmt.executeUpdate("INSERT INTO temp.TimeBins (AtomID, BinnedTime)\n" +
                                           "SELECT AID.AtomID, Time FROM " + getDynamicTableName(
...
//						stmt.executeUpdate("INSERT INTO temp.TimeBins VALUES ("+atomID+",'"+currentBin+"');\n");
     ...
     */
    public boolean createAggregateTimeSeries(
            ProgressBarWrapper progressBar, int rootCollectionID, Collection curColl, int[] mzValues)
        ...
        try {
            ...
            if (curColl.getDatatype().equals("ATOFMS")) {
                if (mzValues == null) {
                    ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                             "Error! Collection: " + collectionName + " doesn't have any peak data to aggregate!");
            ...
            /* IF DATATYPE IS AMS */
                ...
                if (mzValues == null || mzValues.length == 0) {
                    ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                             "Collection: " + collectionName + " doesn't have any peak data to aggregate");
                    System.err.println("Collection: " + collectionID + "  doesn't have any peak data to aggregate");
                    System.err.println("Collections need to overlap times in order to be aggregated.");
                    return false;
                }
            ...
            ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                     "SQL exception aggregating collection: " + collectionName);
            throw new ExceptionAdapter(e);
        }
    }
...
//			if we want to get all mz values:
            ...
            ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                     "SQL exception creating finding M/Z values within collection");
            ...
            for (int i = 0; i < conditionalSeqs.size(); i++) {
                ...
                if (i == 0) {
                    ...
                    if (conditionStrs.get(i).contains("AND")) {
                        collCondStr += "AND (C" + i + ".CollectionID = " + conditionalSeqs.get(i).getCollectionID();
                        collCondStr += " AND " + conditionStrs.get(i).substring(5,
                                                                                conditionStrs.get(i).length()) + ")\n";
                    } else //OR
                    {
                        collCondStr += "OR (C" + i + ".CollectionID = " + conditionalSeqs.get(i).getCollectionID();
                        collCondStr += " AND " + conditionStrs.get(i).substring(4,
                                                                                conditionStrs.get(i).length()) + ")\n";
                    }
                }
    ...
    public ArrayList<TreeMap<Date, Double>> createAndDetectPlumesFromPercent(
            Collection collection, double magnitude, int minDuration) {
    ...
    public ArrayList<TreeMap<Date, Double>> createAndDetectPlumesFromMedian(
            Collection collection, double factor, int minDuration) {
		/*System.out.println("collection: |"+collection+"|");
		System.out.println("Collection ID: "+collection.getCollectionID());
		System.out.println("threshold: "+magnitude+"\nduration: "+minDuration);
		*/
    ...
    public ArrayList<TreeMap<Date, Double>> createAndDetectPlumesFromValue(
            Collection collection, double minValue, int minDuration) {
		/*System.out.println("collection: |"+collection+"|");
		System.out.println("Collection ID: "+collection.getCollectionID());
		System.out.println("threshold: "+magnitude+"\nduration: "+minDuration);
		*/
    ...
    public boolean containsDatatype(String type) {
        ...
        try {
            ...
            ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                     "SQL Exception checking for the existence of datatype " + type);
    ...
    void addInternalAtom(int atomID, int collectionID) {
        try {
            ...
            ErrorLogger.writeExceptionToLogAndPrompt(getName(),
                                                     "SQL Exception adding atom.  Please check incoming data for correct format.");
