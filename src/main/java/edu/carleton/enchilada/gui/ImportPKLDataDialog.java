...
/**
 * Standard importation dialog for PKL data files.
 * 
 * @author PKL importer modified by Michael Murphy 2014
 *
 */
public class ImportPKLDataDialog extends JDialog implements ActionListener{

	private JButton okButton;
	private JButton cancelButton;
	private JCheckBox parentButton;
	private JLabel parentLabel;
	private PKLTableModel PKLTableModel;
	...
	public ImportPKLDataDialog(JFrame owner, Database db) throws HeadlessException {
		...
		JTable PKLTable = getPKLTable();
		JScrollPane scrollPane = new JScrollPane(PKLTable);
    ...
    private JTable getPKLTable()
	{
		PKLTableModel = new PKLTableModel();
		JTable pTable = new JTable(PKLTableModel);		
		TableColumn[] tableColumns = new TableColumn[2];
		for (int i = 0; i < 1; i++)
			tableColumns[i] = pTable.getColumnModel().getColumn(i+1);
		tableColumns[0].setCellEditor(
				new FileDialogPickerEditor("pkl","Dataset",this));
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (source == okButton) {
			...
			final ProgressBarWrapper progressBar = 
				new ProgressBarWrapper(parent, PKLDataSetImporter.TITLE, 100);			
			final PKLDataSetImporter PKL = 
					new PKLDataSetImporter(PKLTableModel, parent, db, progressBar);
