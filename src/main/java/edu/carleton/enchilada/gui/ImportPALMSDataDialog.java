...
/**
 * Standard importation dialog for PALMS data files.
 * 
 * @author rzeszotj with credit to other dialog authors
 *
 */
public class ImportPALMSDataDialog extends JDialog implements ActionListener{

	private JButton okButton;
	private JButton cancelButton;
	private JCheckBox parentButton;
	private JLabel parentLabel;
	private PALMSTableModel PALMSTableModel;
	...
	public ImportPALMSDataDialog(JFrame owner, Database db) throws HeadlessException {
		...
		JTable PALMSTable = getPALMSTable();
		JScrollPane scrollPane = new JScrollPane(PALMSTable);
    ...
    private JTable getPALMSTable()
	{
		PALMSTableModel = new PALMSTableModel();
		JTable pTable = new JTable(PALMSTableModel);		
		TableColumn[] tableColumns = new TableColumn[2];
		for (int i = 0; i < 1; i++)
			tableColumns[i] = pTable.getColumnModel().getColumn(i+1);
		tableColumns[0].setCellEditor(
				new FileDialogPickerEditor("*","Dataset",this));
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (source == okButton) {
			...
			final ProgressBarWrapper progressBar = 
				new ProgressBarWrapper(parent, PALMSDataSetImporter.TITLE, 100);			
			final PALMSDataSetImporter PALMS = 
					new PALMSDataSetImporter(PALMSTableModel, parent, db, progressBar);
			...
			final SwingWorker worker = new SwingWorker() {
				public Object construct() {
					try {
						...
						ErrorLogger.displayException(progressBar,e3.toString());
					}
					return null;
				}
